...
class SimpleOccDenseNet(nn.Module):
    ...
    def __init__(self, hyperparameters, path_to_helper):
        ...
        if "coursenumber" in hyperparameters["features"]:
            ...
            self.enc_size = encoding_dim*self.x_horizon + encoding_dim*self.y_horizon
    ...
    def forward(self, x, y_features, additional_info):
        ...
        y_features = self.flatten_layer(y_features)
    ...
    def forecast_iter(self, x, y_features, len_y, additional_info):
        ...
        for i in range(0, len_y, self.y_horizon):
            ...
            if "coursenumber" in self.hyperparameters["features"]:
                ...
                x_i_in = torch.cat((x_i, x_emb_i), -1)
                y_feat_i_in= torch.cat((y_feat_i, y_emb_i), -1)

            else:
                x_i_in = x_i
                y_feat_i_in = y_feat_i
                
            x_i_in = self.flatten_layer(x_i_in)
            y_feat_i_in = self.flatten_layer(y_feat_i_in)
            
            input = torch.cat((x_i_in, y_feat_i_in), 1)
            ...
            if self.hyperparameters["include_x_features"]:
                x_new = torch.cat((out[:, :, None], y_feat_i), dim=2)
            else:
                x_new = out[:, :,  None]
                

            x_i = torch.cat((x_i, x_new), dim=1)[:, self.y_horizon:]
...
class MultiHeadOccDenseNet(nn.Module):
    ...
    def __init__(self, hyperparameters, path_to_helper):
        ...
        self.input_size_1 = self.x_size + encoding_dim*self.x_horizon 
        self.input_size_2 = self.y_features_size + encoding_dim*self.y_horizon
        
        
        self.x_in = nn.Sequential(
            nn.Linear(self.input_size_1, self.hidden_size[0]),
            nn.ReLU(),
            nn.Linear(self.hidden_size[0], self.hidden_size[0])
        )
        self.y_in = nn.Sequential(
            nn.Linear(self.input_size_2, self.hidden_size[0]),
            nn.ReLU(),
            nn.Linear(self.hidden_size[0], self.hidden_size[0])
        )
        ...
        hidden_size_out = self.hidden_size[1:]
        
        if len(hidden_size_out) > 0:
            self.out_head.add_module("input_layer", nn.Linear(self.hidden_size[0]*2, self.hidden_size[1]))
            self.out_head.add_module("relu_0", nn.ReLU())
            self.out_head.add_module("output_layer", nn.Linear(self.hidden_size[1], self.output_size))

            
        else:
            self.out_head.add_module("input_layer", nn.Linear(self.hidden_size[0]*2, self.output_size))
    ...
    def forward(self, x, y_features, additional_info):
        ...
        x = self.x_in(x)
        y_features = self.y_in(y_features)
        
        input = torch.cat((x, y_features), 1)

        out = self.out_head(input)
    ...
    def forecast_iter(self, x, y_features, len_y, additional_info):
        ...
        for i in range(0, len_y, self.y_horizon):
            ...
            x_i_in = self.flatten_layer(x_i_in)
            x_i_enc = self.x_in(x_i_in)
            
            y_feat_i_in = self.flatten_layer(y_feat_i_in)
            y_feat_i_enc = self.y_in(y_feat_i_in)
            
            input = torch.cat((x_i_enc, y_feat_i_enc), 1)
...
class SimpleOccLSTM(torch.nn.Module):
    ...
    def __init__(self, hyperparameters, path_to_helper, **kwargs):
        ...
        if self.hyperparameters["prelayer"]:  
            ...
            self.lstm_input_size = self.hidden_size[0]
        
        else: 
            self.lstm_input_size = hyperparameters["x_size"] + encoding_dim
        ...
        if len(self.hidden_size) == 2:
            self.fc_end.add_module("input_layer", nn.Linear(self.hidden_size[0]*self.bdir_factor, self.hidden_size[1]))
    ...
    def forward(self, x, y_features, room_id=None):
        ...
        for i in range(0, self.hyperparameters["y_horizon"]):
            ...
            if self.hyperparameters["prelayer"]:
                y_in = self.fc_in_enc(y_in)
                
            h_t1, (h_n, c_n) = self.lstm(y_in, (h_n, c_n))
            y_t = self.fc_end(h_t1)
