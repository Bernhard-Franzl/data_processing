...
comb_iterator = ParameterSearch(
    path_to_json=path_to_params
).grid_search_iterator(tqdm_bar=True)
...
for n_comb, hyperparameters in enumerate(comb_iterator, start=start_comb):
    ...
    train_loader, val_loader, test_loader, model, optimizer = mt.intialize_all(
        dataframes, indices, hyperparameters["dataset_mode"], helper_path)
